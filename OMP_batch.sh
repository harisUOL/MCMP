...
OUT1=$6_output_gen_c.txt
OUT2=$6_output_gen_f.txt
OUT3=$6_output_gen_n.txt
...
gcc -fopenmp -O0 $SRC $A1 $A2 $A3 $COMM -o $EXE -std=c99 -lm
echo
echo ------------------------------------
...
if test -x $EXE; then 
      ...
      for i in {1..1}; do ./${EXE} $TEST1 $OUT1 $OUT2 $OUT3; done
else
