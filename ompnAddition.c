...
int *solveNearestAddition(double **dMatrix, int numOfCoords, int start){
	...
	//Get the env variable for the max num of threads.
	...
	#pragma omp parallel 
	{
		...
		while(numVisited < numOfCoords){
			...
			#pragma omp single
			{
				...
				//Check added cost of both possibilities
				if(bestInsertPos == 1)
				{
					double startAddCost = dMatrix[tour[0]][bestNextNode] + dMatrix[tour[1]][bestNextNode] - dMatrix[tour[0]][tour[1]];
					double endAddCost = dMatrix[tour[(tourLength - 2)]][bestNextNode] + dMatrix[tour[(tourLength - 1)]][bestNextNode] - dMatrix[tour[(tourLength - 2)]][tour[(tourLength - 1)]];
					if(endAddCost < startAddCost)
						bestInsertPos = (tourLength - 1);
				}
				else
				{
					double befAddCost = dMatrix[tour[(bestInsertPos-2)]][bestNextNode] + dMatrix[tour[(bestInsertPos-1)]][bestNextNode] - dMatrix[tour[(bestInsertPos-2)]][tour[(bestInsertPos-1)]];
					double afAddCost = dMatrix[tour[(bestInsertPos-1)]][bestNextNode] + dMatrix[tour[bestInsertPos]][bestNextNode] - dMatrix[tour[(bestInsertPos-1)]][tour[bestInsertPos]];
					if(befAddCost <= afAddCost)
						bestInsertPos--;
				}
